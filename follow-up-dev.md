...
- [ ] **Completion Tracking:** Allow users to set a target completion date for the job. A progress bar appears on the dashboard job card to indicate how much time is left to hit the target completion date. The progress bar should be color coded to indicate how much time is left. Red for critical, yellow for warning, green for good. 0-30% = Green. 31%-70% yellow. 71%-100% red The progress bar progression will be set based on how far away the completion date is and it will progress in 5% chunks. For example if i created a job today and set the target completion date 10 days from now, the progress bar would move 5% every 12 hours. 
- [ ] **Status:** Allow users to update the Job Status. These color status will be shown via a thin border around the outside of the job card on the dashboard. 0/Red status = quoted only, 1/Yellow status = project started but delayed, 2/Green status = work in progress, 3/Blue status = punchlist ony, 4/Purple status = Final bill submitted, 5/Glowing purple blue green status = job is complete payment received. Ready for archive.
